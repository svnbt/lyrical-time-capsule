...
# 1. Konfiguration für ein nahtloses Vollbild-Erlebnis
st.set_page_config(layout="wide", page_title="The Lyrical Time Capsule", initial_sidebar_state="collapsed")

# 2. Streamlit-Ränder und Header verstecken, damit das HTML den ganzen Platz hat
st.markdown("""
    <style>
...
# 3. Hilfsfunktion, um Bilder in lesbaren Code (Base64) umzuwandeln
def get_base64_of_bin_file(bin_file):
    with open(bin_file, 'rb') as f:
        data = f.read()
    return base64.b64encode(data).decode()

# 4. HTML, CSS und Bilder laden und zusammenbauen
try:
    # HTML einlesen
    with open("article.html", "r", encoding="utf-8") as f:
        html_content = f.read()

    # CSS direkt in das HTML injizieren. 
    ...
    # ==========================================
    # NEU: DER FONT-HACK FÜR AKIRA
    # ==========================================
    akira_path = "style/fonts/akira.otf"
    if os.path.exists(akira_path):
        b64_akira = get_base64_of_bin_file(akira_path)
        
        # Ersetze den Pfad im CSS durch den echten Base64-Code der Schriftart
        # HINWEIS: Achte darauf, dass der zu ersetzende String exakt dem entspricht, was in deiner theme.css steht!
    ...
    # NEU: Das Animations-CSS injizieren
    for link in alte_animation_links:
        html_content = html_content.replace(link, f'<style>{animation_css}</style>')


    # NEU: Den Link dynamisch umleiten auf die Streamlit-Unterseite
    html_content = html_content.replace('href="prompts.html"', 'href="Prompts" target="_top"')
    # Falls in deinem HTML noch target="_blank" steht, löschen wir das sicherheitshalber raus:
    html_content = html_content.replace('target="_blank" target="_top"', 'target="_top"')

    # Bilder in Base64 umwandeln und im HTML ersetzen
    charts = ["chart1.png", "chart2.png", "chart3.png", "chart4.png"]
    for chart in charts:
        img_path = f"chartsPNG/{chart}"
        if os.path.exists(img_path):
            b64 = get_base64_of_bin_file(img_path)
            # Ersetzt den src-Pfad durch den generierten Bild-Code
            html_content = html_content.replace(f'src="chartsPNG/{chart}"', f'src="data:image/png;base64,{b64}"')

    # 5. Das fertige HTML in Streamlit anzeigen
